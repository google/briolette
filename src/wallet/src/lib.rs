...
mod tests {
    use super::*;
    use briolette_proto::briolette::{ ServiceName, ServiceMapInterface };
    ...
    async fn setup_validate(clerk_uri: String, tokenmap_uri: String) -> String {
        ...
        tokio::spawn(async move {
            ...
            let epoch_pk = std::fs::read("../../data/clerk/epoch.pk")
                .expect("../../data/clerk not populated yet");
            let mut epoch_update = BrioletteValidate::fetch_epoch_update(&clerk_uri, &epoch_pk).await;
            ...
            eed.service_map.add(ServiceName::Tokenmap, &tokenmap_uri);
            let known_tms = eed.service_map.get(ServiceName::Tokenmap);
            eu.extended_data = Some(eed.clone());

            println!("Configuring validation against test tokenmap: {}", tokenmap_uri);
            println!("Known tokenmaps: {:?}", known_tms);

            let validate = BrioletteValidate::new(&eu)
                .await
                .unwrap();
            let listener = TcpListener::bind("[::1]:0").await.unwrap();
            let addr = listener.local_addr().unwrap();
